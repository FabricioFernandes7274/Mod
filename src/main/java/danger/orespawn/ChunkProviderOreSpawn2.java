...
public class ChunkProviderOreSpawn2 implements IChunkGenerator {
    ...
    private List<Biome.SpawnListEntry> myMobList = null;
    private List<Biome.SpawnListEntry> myAmbientList = null;
    ...
    public ChunkProviderOreSpawn2(World worldIn, long seed, boolean mapFeaturesEnabled) {
        this.world = worldIn;
        ...
        // Inicializa geradores através dos Eventos do Forge
    ...
    public void populate(int x, int z) {
        ...
        if (this.mapFeaturesEnabled) {
            ...
            flag = this.strongholdGenerator.generateStructure(this.world, this.rand, chunkpos);
            this.scatteredFeatureGenerator.generateStructure(this.world, this.rand, chunkpos);
        }

        // Geração de Lagos (Água e Lava) e Dungeons usando os eventos do Forge
        ...
        // Decora o Bioma com as suas árvores/ervas habituais
        biome.decorate(this.world, this.rand, blockpos);

        // Gera os minérios/spawners do OreSpawn para a Village Dimension
        ...
        if (OreSpawnMain.LessOre == 0) {
            // Se LessOre == 0, gera 3 VEZES MAIS (Mantendo o comportamento absurdo da Village Dimension)
            OreSpawnMain.Chunker.generateOresInChunk(this.world, this.rand, i, j, chunk);
            OreSpawnMain.Chunker.generateOresInChunk(this.world, this.rand, i, j, chunk);
        }

        // Geração de Gelo e Neve
    ...
    public List<Biome.SpawnListEntry> getPossibleCreatures(EnumCreatureType creatureType, BlockPos pos) {
        ...
        List<Biome.SpawnListEntry> l = biome.getSpawnableList(creatureType);
        if (l == null) l = new ArrayList<>();

        if (creatureType == EnumCreatureType.MONSTER) {
            if (this.myMobList == null) {
                this.myMobList = new ArrayList<>(l);
                if (OreSpawnMain.AlosaurusEnable != 0) this.myMobList.add(new Biome.SpawnListEntry(Alosaurus.class, 8, 1, 2));
                if (OreSpawnMain.TRexEnable != 0) this.myMobList.add(new Biome.SpawnListEntry(TRex.class, 6, 1, 2));
                if (OreSpawnMain.NastysaurusEnable != 0) this.myMobList.add(new Biome.SpawnListEntry(Nastysaurus.class, 6, 1, 2));
                if (OreSpawnMain.PointysaurusEnable != 0) this.myMobList.add(new Biome.SpawnListEntry(Pointysaurus.class, 10, 4, 8));
                if (OreSpawnMain.GammaMetroidEnable != 0) this.myMobList.add(new Biome.SpawnListEntry(GammaMetroid.class, 35, 4, 7));
                if (OreSpawnMain.AlienEnable != 0) this.myMobList.add(new Biome.SpawnListEntry(Alien.class, 35, 2, 3));
                if (OreSpawnMain.CaveFisherEnable != 0) this.myMobList.add(new Biome.SpawnListEntry(CaveFisher.class, 35, 4, 8));
                if (OreSpawnMain.CryolophosaurusEnable != 0) this.myMobList.add(new Biome.SpawnListEntry(Cryolophosaurus.class, 26, 4, 7));
                if (OreSpawnMain.SpyroEnable != 0) this.myMobList.add(new Biome.SpawnListEntry(Spyro.class, 5, 1, 2));
            }
        ...
        if (creatureType == EnumCreatureType.AMBIENT) {
            if (this.myAmbientList == null) {
                ...
                for (Biome.SpawnListEntry entry : l) {
                    if (entry.entityClass != EntityHorse.class) { // Impede spawn natural de cavalos vainilla para priorizar do OreSpawn
                        this.myAmbientList.add(entry);
                    }
                }
                if (OreSpawnMain.VelocityRaptorEnable != 0) this.myAmbientList.add(new Biome.SpawnListEntry(VelocityRaptor.class, 1, 2, 4));
                if (OreSpawnMain.DragonflyEnable != 0) this.myAmbientList.add(new Biome.SpawnListEntry(Dragonfly.class, 2, 1, 3));
                if (OreSpawnMain.CamarasaurusEnable != 0) this.myAmbientList.add(new Biome.SpawnListEntry(Camarasaurus.class, 1, 2, 4));
                if (OreSpawnMain.BaryonyxEnable != 0) this.myAmbientList.add(new Biome.SpawnListEntry(Baryonyx.class, 2, 4, 8));
            }
