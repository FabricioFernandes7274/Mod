...
extends ModelBase {
    ...
    public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
        ...
        float newangle = 0.0f;
        newangle = (double)f1 > 0.1 ? net.minecraft.util.math.MathHelper.cos((float)(f2 * this.wingspeed * 1.4f)) * (float)Math.PI * 0.55f * f1 : 0.0f;
        ...
        newangle = c.getSinging() != 0 ? net.minecraft.util.math.MathHelper.cos((float)(f2 * 0.85f * this.wingspeed)) * (float)Math.PI * 0.15f : 0.0f;
        this.jaw.rotateAngleX = newangle + 1.22f;
        if (c.motionY > (double)0.1f || c.motionY < (double)-0.1f) {
