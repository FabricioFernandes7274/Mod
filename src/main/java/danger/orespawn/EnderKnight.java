...
public class EnderKnight extends EntityMob {
    ...
    // Sincronização para a animação da boca aberta (Enderman style)
    private static final DataParameter<Boolean> SCREAMING = EntityDataManager.createKey(EnderKnight.class, DataSerializers.BOOLEAN);
    ...
    protected void applyEntityAttributes() {
        ...
        // Valores base do OreSpawn (ajuste se não usar a classe de stats)
    ...
    public void onLivingUpdate() {
        ...
        // Lógica de Speed Boost ao perseguir alvo
        ...
        // Lógica de Teleporte e Encarar Player
        if (!this.world.isRemote && this.isEntityAlive()) {
            EntityPlayer player = this.world.getClosestVulnerablePlayerToEntity(this, 64.0D);
            ...
            // Teleporte se estiver muito longe ou muito perto do alvo
            if (this.getAttackTarget() != null) {
    ...
    private boolean shouldAttackPlayer(EntityPlayer player) {
        ItemStack itemstack = player.inventory.armorInventory.get(3); // Capacete
    ...
    private boolean teleportTo(double x, double y, double z) {
        ...
        if (flag) {
            this.world.playSound(null, this.prevPosX, this.prevPosY, this.prevPosZ, SoundEvents.ENTITY_ENDERMEN_TELEPORT, SoundCategory.HOSTILE, 1.0F, 1.0F);
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        ...
        if (source.isProjectile() && !this.world.isRemote) this.teleportRandomly();
        return flag;
    }

    @Override
    protected Item getDropItem() { return Items.ENDER_PEARL; }
    ...
    // Sons customizados (usando sons padrão do Enderman para facilitar)
    @Override
