...
extends EntityMob {
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue((double)OreSpawnMain.Vortex_stats.attack);
    }
    ...
    protected void updateAITasks() {
        ...
        if (this.getEntityWorld().rand.nextInt(300) == 0 || this.currentFlightTarget.getDistanceSquared((int)this.posX, (int)this.posY, (int)this.posZ) < 2.1f) {
            ...
            while (bid != Blocks.AIR && keep_trying != 0) {
                zdir = this.getEntityWorld().rand.nextInt(14) + 10;
                xdir = this.getEntityWorld().rand.nextInt(14) + 10;
        ...
        if ((e = this.findSomethingToAttack()) != null) {
            ...
            if (d < 81.0 && this.winded == 0) {
                ...
                e.addVelocity(Math.cos(a) * (10.0 - Math.sqrt(d)) * (double)0.1f, (10.0 - Math.sqrt(d)) * (double)0.05f * pm, Math.sin(a) * (10.0 - Math.sqrt(d)) * (double)0.1f);
            }
            if (this.getDistanceSq((Entity)e) < (double)((4.0f + e.width / 2.0f) * (4.0f + e.width / 2.0f)) && this.getEntityWorld().rand.nextInt(8) == 2) {
