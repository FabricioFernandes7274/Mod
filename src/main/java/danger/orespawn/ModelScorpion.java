...
extends ModelBase {
    ...
    public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
        ...
        newangle = net.minecraft.util.math.MathHelper.cos((float)(f2 * 2.0f * this.wingspeed - 1.0f * pi4)) * (float)Math.PI * 0.12f * f1;
        this.lleg2.rotateAngleY = newangle + 0.24f;
        this.rleg2.rotateAngleY = -newangle + 2.9f;
        newangle = net.minecraft.util.math.MathHelper.cos((float)(f2 * 2.0f * this.wingspeed - 2.0f * pi4)) * (float)Math.PI * 0.12f * f1;
        this.lleg3.rotateAngleY = newangle - 0.24f;
        this.rleg3.rotateAngleY = -newangle - 2.9f;
        newangle = net.minecraft.util.math.MathHelper.cos((float)(f2 * 2.0f * this.wingspeed - 3.0f * pi4)) * (float)Math.PI * 0.12f * f1;
        this.lleg4.rotateAngleY = newangle - 0.49f;
        this.rleg4.rotateAngleY = -newangle - 2.65f;
        r = e.getRenderInfo();
        newangle = net.minecraft.util.math.MathHelper.cos((float)(f2 * 3.0f * this.wingspeed)) * (float)Math.PI * 0.15f;
        nextangle = net.minecraft.util.math.MathHelper.cos((float)((f2 + 0.1f) * 3.0f * this.wingspeed)) * (float)Math.PI * 0.15f;
        if (nextangle > 0.0f && newangle < 0.0f) {
            r.ri1 = 0;
            if (e.getAttacking() == 0) {
                r.ri1 = e.world.rand.nextInt(20);
                r.ri2 = e.world.rand.nextInt(25);
            } else {
                r.ri1 = e.world.rand.nextInt(4);
                r.ri2 = e.world.rand.nextInt(3);
            }
        }
        if (r.ri1 == 1 || r.ri1 == 3) {
