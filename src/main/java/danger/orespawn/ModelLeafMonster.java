...
extends ModelBase {
    ...
    public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
        ...
        if (lm.getAttacking() == 0) {
            ...
            this.rarm.rotationPointY = -8.0f;
            this.larm.rotationPointY = -8.0f;
            float newangle = (double)f1 > 0.1 ? net.minecraft.util.math.MathHelper.cos((float)(f2 * 0.95f)) * (float)Math.PI * 0.25f * f1 : 0.0f;
            this.lleg.rotateAngleX = newangle;
            this.rleg.rotateAngleX = -newangle;
            newangle = net.minecraft.util.math.MathHelper.cos((float)(f2 * 0.7f)) * (float)Math.PI * 0.55f;
